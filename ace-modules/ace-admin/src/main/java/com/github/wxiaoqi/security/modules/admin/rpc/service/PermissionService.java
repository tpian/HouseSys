...
public class PermissionService {
    ...
    public void save(User user) {
        ...
        Userinfo userByPhone = userinfoBiz.UserByPhone(userinfo.getMobilePhone());
        if (userByPhone != null) {
            throw new UserInvalidException("手机号已注册");
        }
        if (userinfo.getType() == 1) {
            userinfo.setAddress(userinfo.getCounty() + userinfo.getTown() + userinfo.getVillage());
        } else if (userinfo.getType() == 2) {
            userinfo.setAddress(userinfo.getCounty() + userinfo.getTown());
        } else {
            userinfo.setAddress(userinfo.getCounty());
        }
        userinfoBiz.insertSelective(userinfo);

        User userByUsername = userBiz.getUserByUsername(user.getUsername());
        if (userByUsername != null) {
            throw new UserInvalidException("用户已存在");
        }

        if (user.getType() == 1) {
            user.setAddress(user.getCounty() + user.getTown() + user.getVillage());
        } else if (user.getType() == 2) {
            user.setAddress(user.getCounty() + user.getTown());
        } else {
            user.setAddress(user.getCounty());
        }
    ...
    public void update(User user) {
        ...
        Userinfo userByPhone = userinfoBiz.UserByPhone(user.getMobilePhone());
        if (userByPhone != null) {
            userByPhone.setName(user.getName());
            userByPhone.setAddress(user.getAddress());
            userByPhone.setSex(user.getSex());
            userByPhone.setCounty(user.getCounty());
            userByPhone.setTown(user.getTown());
            userByPhone.setVillage(user.getVillage());
            userByPhone.setUnit(user.getUnit());
            userByPhone.setType(user.getType());
            userinfoBiz.updateSelectiveById(userByPhone);
        }

    }

    public void dluser(int id) {
        User user = userBiz.selectById(id);
        Userinfo userByPhone = userinfoBiz.UserByPhone(user.getMobilePhone());
        if (userByPhone != null) {
            userByPhone.setIsDelete(1);
            userinfoBiz.updateSelectiveById(userByPhone);
        }
    ...
    public UserInfo valiphone(String phone, String password) {
        UserInfo info = new UserInfo();
        User user = userBiz.UserByPhone(phone);
        if (user == null) {
            throw new UserInvalidException("用户不存在!");
        }
        if (!encoder.matches(password, user.getPassword())) {
        ...
        if (encoder.matches(password, user.getPassword())) {
            BeanUtils.copyProperties(user, info);
            info.setId(user.getId().toString());
            info.setPhone(user.getMobilePhone());
        }
        return info;
    }
